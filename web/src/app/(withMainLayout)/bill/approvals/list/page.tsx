...
import { CheckBox, PageLoader } from "@rever/common";
...
const ApprovalList = () => {
  ...
  useEffect(() => {
    if (underApprovalBills && underApprovalBills.length) {
      const structuredData = underApprovalBills?.map(
        (val: ApprovalListAPIType) => ({
          ...
          due_date:
            val?.due_date && formatDate(val?.due_date, orgDetails?.date_format),
          vendor: val?.vendor,
          total: val?.total
