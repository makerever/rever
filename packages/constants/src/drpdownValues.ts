...
// Country dropdown options (from country-state-city package)
export const countryOptions = Country.getAllCountries().map((c) => ({
  id: c.isoCode.toLowerCase(),
  label: c.name,
  value: c.isoCode.toLowerCase(),
}));

// State dropdown options (linked to countryOptions by countryId)
export const stateOptions = State.getAllStates().map((s) => ({
  id: s.isoCode.toLowerCase(),
  label: s.name,
  value: s.isoCode.toLowerCase(),
  countryId: s.countryCode.toLowerCase(),
}));

// City dropdown options (linked to stateOptions by stateId)
export const cityOptions = City.getAllCities().map((ct) => ({
  id: `${ct.name
    .toLowerCase()
    .replace(/\s+/g, "-")}-${ct.countryCode.toLowerCase()}`,
  label: ct.name,
  value: ct.name.toLowerCase().replace(/\s+/g, "-"),
  stateId: ct.stateCode.toLowerCase(),
  stateCode: ct.countryCode.toLowerCase(),
}));

// Theme mode dropdown options
